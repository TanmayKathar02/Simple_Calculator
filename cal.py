...
def get_result():
    ...
    if operator == '+':
        ...
        result_label.config(text=str((first_number /100) * second_number))
    else:
        if second_number == 0:
...
root = Tk()
root.title('Calcaulator')
root.geometry('280x445')
root.resizable(0,0)
root.configure(background='black')

result_label = Label(root,text='',bg='black',fg='orange')
...
btn_mul= Button(root,text='*',bg='white',fg='black',width=5,height=2,command=lambda :get_operator('*'))
...
btn_sub= Button(root,text='-',bg='white',fg='black',width=5,height=2,command=lambda :get_operator('-'))
...
btn_dub= Button(root,text='00',bg='white',fg='black',width=5,height=2,command=lambda :add_double_zero())
btn_dub.grid(row=5,column=0)
btn_dub.config(font=('verdana',14))

btn0= Button(root,text='0',bg='white',fg='black',width=5,height=2,command=lambda :get_digit(0))
btn0.grid(row=5,column=1)
btn0.config(font=('verdana',14))

btn_dot= Button(root,text='.',bg='white',fg='black',width=5,height=2,command=lambda :add_dot())
btn_dot.grid(row=5,column=2)
btn_dot.config(font=('verdana',14))

btn_equal= Button(root,text='=',bg='#7ae874',fg='black',width=5,height=2,command=get_result)
btn_equal.grid(row=5,column=3)
btn_equal.config(font=('verdana',14))

btn_clr= Button(root,text='C',bg='#ed1d15',fg='black',width=5,height=2,command=lambda :clear())
btn_clr.grid(row=1,column=0)
btn_clr.config(font=('verdana',14))

btn_per= Button(root,text='%',bg='white',fg='black',width=5,height=2,command=lambda :get_operator('%'))
btn_per.grid(row=1,column=1)
btn_per.config(font=('verdana',14))

btn_del= Button(root,text='Del',bg='#a5a8a8',fg='black',width=5,height=2,command=lambda :delete_digit())
btn_del.grid(row=1,column=2)
btn_del.config(font=('verdana',14))

btn_div= Button(root,text='/',bg='white',fg='black',width=5,height=2,command=lambda :get_operator('/'))
